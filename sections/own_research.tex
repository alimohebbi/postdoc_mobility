...
%, which collaborate to migrate source test case $t^s$ to target test case $t^t$.
...
%\testreuse approaches implement the components differently.
...
%The \wem encodes words to vectors, representing the semantic distance of the words.
...
%that we collected from crawling around 1 million apps in Google Play. 
...
%Because the \tam is too incomplete to find a matching event in outside the current state or a path to the matching event.
%Because the \tam is too incomplete to find a path to a  matching event outside the current state.
